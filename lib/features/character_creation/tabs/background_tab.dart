...
class _BackgroundTabState extends State<BackgroundTab> {
  ...
  List<bool> get backgroundSelectedSkills {
    List<String> options =
        widget.character.background.optionalSkillProficiencies;
    return options
        .map((x) => widget.character.skillsSelected.contains(x))
        .toList();
  }
  ...
  List<bool> get backgroundSelectedLanguages {
    ...
    return options
        .map((x) => widget.character.languageChoices.contains(x))
        .toList();
  }
              ...
              children: [
                ...
                StyleUtils.buildStyledDropDown(
                    ...
                    onChanged: (String? value) {
                      setState(() {
                        widget.character.background = GlobalListManager()
                            .backgroundList
                            .singleWhere((x) => x.name == value);
                        widget.character.backgroundPersonalityTrait =
                            widget.character.background.personalityTrait.first;
                        widget.character.backgroundIdeal =
                            widget.character.background.ideal.first;
                        widget.character.backgroundBond =
                            widget.character.background.bond.first;
                        widget.character.backgroundFlaw =
                            widget.character.background.flaw.first;
                ...
                StyleUtils.buildLabeledDropdown(
                    ...
                    onChanged: (String? value) {
                      setState(() {
                        widget.character.backgroundPersonalityTrait = widget
                            .character.background.personalityTrait
                ...
                StyleUtils.buildLabeledDropdown(
                    ...
                    onChanged: (String? value) {
                      setState(() {
                        widget.character.backgroundIdeal = widget
                ...
                StyleUtils.buildLabeledDropdown(
                    ...
                    onChanged: (String? value) {
                      setState(() {
                        widget.character.backgroundBond = widget
                ...
                StyleUtils.buildLabeledDropdown(
                    ...
                    onChanged: (String? value) {
                      setState(() {
                        widget.character.backgroundFlaw = widget
                ...
                if (widget.character.background.numberOfSkillChoices != 0 &&
                    widget.character.background.optionalSkillProficiencies
                        .isNotEmpty) ...[
                  StyleUtils.buildStyledSmallTextBox(
                      text:
                          "Pick ${(widget.character.background.numberOfSkillChoices)} skill(s) to gain proficiency in"),
                  const SizedBox(height: 7),
                  StyleUtils.buildStyledToggleSelector(
                      options: widget
                          .character.background.optionalSkillProficiencies,
                      ...
                      onToggle: (String skill) {
                        setState(() {
                          if (widget.character.skillsSelected.contains(skill)) {
                            ...
                            if (widget.character.skillsSelected.length >
                ...
                if (widget.character.background.numberOfLanguageChoices !=
                    0) ...[
                  StyleUtils.buildStyledSmallTextBox(
                      text:
                          "Pick ${(widget.character.background.numberOfLanguageChoices)} language(s) to learn"),
                  const SizedBox(height: 7),
                  StyleUtils.buildStyledToggleSelector(
