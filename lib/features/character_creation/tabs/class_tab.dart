...
class _ClassTabState extends State<ClassTab> {
  ...
  bool multiclassingPossible(Class selectedClass) {
    ...
    if (widget.character.classList.isEmpty ||
        widget.character.classList.contains(selectedClass.name)) {
      return true;
    }
    ...
    if (scoresFailRequirement(
        widget.character, selectedClass.multiclassingRequirements)) {
      return false;
    }
  ...
  Widget? levelGainParser(List<dynamic> x, Class selectedClass) {
    ...
    if (x[0] == "Level") {
      ...
      widget.character.skillBonusMap[x[1]] =
          widget.character.skillBonusMap[x[1]]! + int.parse(x[2]);
      //do this later
    } else if (x[0] == "ASI") {
      widget.onNumberOfRemainingFeatOrASIsChanged(
          widget.numberOfRemainingFeatOrASIs + 1);
    }
    ...
    SPEEDLIST.append([x[1], x[2]]);
      ...
      widget.character.currency[x[1]] =
          widget.character.currency[x[1]]! + int.parse(x[2]);
    }
    return null;
  }
  ...
  int levelZeroGetSpellsKnown(int index) {
    if (GlobalListManager().classList[index].spellsKnownFormula == null) {
      return GlobalListManager()
          .classList[index]
          .spellsKnownPerLevel![widget.character.classLevels[index]];
    }
    //decode as zero
    return 3;
  }
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Wrap(
                          ...
                          children: List.generate(
                              GlobalListManager().classList.length, (index) {
                            return Container(
                                width: 240,
                                height: 175,
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    StyleUtils.buildStyledTinyTextBox(
                                        text:
                                            "Class type: ${GlobalListManager().classList[index].classType}",
                                        color: ThemeManager
                                            .instance.currentScheme.textColour),
                                    StyleUtils.buildStyledTinyTextBox(
                                        text: (["Martial", "Third Caster"]
                                                .contains(GlobalListManager()
                                                    .classList[index]
                                                    .classType))
                                    ...
                                    StyleUtils.buildStyledTinyTextBox(
                                        text:
                                            "Hit die: D${GlobalListManager().classList[index].maxHitDiceRoll}",
                                        color: ThemeManager
                                            .instance.currentScheme.textColour),
                                    StyleUtils.buildStyledTinyTextBox(
                                        text:
                                            "Saves: ${GlobalListManager().classList[index].savingThrowProficiencies.join(", ")}",
                                        color: ThemeManager
                                            .instance.currentScheme.textColour),
                                    ...
                                    ElevatedButton(
                                        style: OutlinedButton.styleFrom(
                                            backgroundColor: (widget.charLevel <=
                                                        widget.character
                                                            .classLevels
                                                            .reduce(sum) ||
                                        ...
                                        onPressed: () {
                                          setState(() {
                                            List<Widget> newWidgetsInPlay =
                                                List.from(widget.widgetsInPlay);
                                            ...
                                            if (widget.charLevel >
                                                ...
                                                (multiclassingPossible(
                                                    GlobalListManager()
                                                        .classList[index]))) {
                                              widget.character.classList.add(
                                                  GlobalListManager()
                                                      .classList[index]
                                                      .name);
                                              ...
                                              if (GlobalListManager()
                                                ...
                                                newWidgetsInPlay.add(StyleUtils
                                                    .buildStyledSmallTextBox(
                                                        text:
                                                            "No choices needed for ${GlobalListManager().classList[index].name} level ${GlobalListManager().classList[index].gainAtEachLevel[widget.character.classLevels[index]][0][1]}"));
                                              } else {
                                                newWidgetsInPlay.add(StyleUtils
                                                    .buildStyledMediumTextBox(
                                                        text:
                                                            "${GlobalListManager().classList[index].name} Level ${GlobalListManager().classList[index].gainAtEachLevel[widget.character.classLevels[index]][0][1]} choice(s):"));
                                              }

                                              for (List<dynamic> x
                                                  in GlobalListManager()
                                                          .classList[index]
                                                          .gainAtEachLevel[
                                                      widget.character
                                                              .classLevels[
                                              ...
                                              if (widget.character.classList
                                                ...
                                                if (widget.character
                                                    ...
                                                    false) {
                                                  widget.onNumberOfRemainingFeatOrASIsChanged(
                                                      widget.numberOfRemainingFeatOrASIs +
                                                          1);
                                                }
                                                ...
                                                newWidgetsInPlay.addAll([
                                                  StyleUtils
                                                      .buildStyledSmallTextBox(
                                                          text:
                                                              "Pick ${(GlobalListManager().classList[index].numberOfSkillChoices)} skill(s) to gain proficiency in"),
                                                  const SizedBox(height: 7),
                                                    ...
                                                    if (widget.character
                                                      ...
                                                      widget.character
                                                          ...
                                                          getSpellsKnown(
                                                              index,
                                                              widget.character
                                                                  .allSpellsSelectedAsListsOfThings[x]);
                                                    } else if (a != null) {
                                                      if (widget.character
                                                                  .allSpellsSelectedAsListsOfThings[
                                                              x][0] ==
