                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: StyleUtils.buildBaseDropdownButton(
                                value: GlobalListManager()
                                        .groupList
                                        .contains(widget.character.group)
                                    ? widget.character.group
                                    : null,
                                items: GlobalListManager().groupList.isNotEmpty
                                    ? GlobalListManager().groupList
                                    : null,
                                ...
                                hintText:
                                    (GlobalListManager().groupList.isNotEmpty)
                          ...
                          Tooltip(
                              message: widget.canCreateCharacter
                                  ? (widget.isEditMode
                                      ? "This button will save your character edits and return you to the main menu."
                                      : "This button will save your character putting it into the Json and then send you back to the main menu.")
                                  : (widget.isEditMode
                                      ? "You must complete the required tabs before saving your character edits"
                                      : "You must complete the required tabs before saving your character"),
                              child: ElevatedButton(
                                style: OutlinedButton.styleFrom(
                                  backgroundColor: widget.canCreateCharacter
                                      ? ThemeManager
                                          .instance.currentScheme.backingColour
                                      : const Color.fromARGB(247, 56, 53, 52),
                                  padding:
                    ...
                    child: Column(spacing: 20, children: [
                      ...
                      if (!widget.isEditMode)
                        StyleUtils.makeOptionalText(
                            condition: widget.character.basicsComplete,
                            trueText: "Filled in all basic information",
                            falseText:
                                "Haven't filled in all necessary basics"),

                      // Show ability scores check only in creation mode
                      if (!widget.isEditMode)
                        StyleUtils.makeRequiredText(
                            condition: (widget.pointsRemaining == 0),
                            trueText: "Used all ability score points",
                            falseText:
                                "${widget.pointsRemaining} unspent ability score points"),
                      ...
                      (widget.numberOfRemainingFeatOrASIs == 0)
                          ...
                          : StyleUtils.buildStyledMediumTextBox(
                              text:
                                  "You have ${widget.numberOfRemainingFeatOrASIs} ASI/Feat (s) remaining",
                              color: negativeColor),

                      //Class
                      StyleUtils.makeRequiredText(
                          condition: (widget.charLevel <=
                              widget.character.classList.length),
                          trueText: "Made all level selections",
                          falseText:
                              "${widget.charLevel - widget.character.classList.length} unused level${(widget.charLevel > 1) ? "s" : ""}"),
                      ...
                      StyleUtils.makeRequiredText(
                          ...
                          falseText:
                              "Missed ${widget.character.equipmentSelectedFromChoices.where((element) => element.length == 2).toList().length} equipment choice(s)"),
                      ...
                      StyleUtils.makeRequiredText(
                          ...
                          falseText:
                              "Missed ${(widget.character.allSpellsSelectedAsListsOfThings.fold(0, (a, b) => a + (b[2] as int)))} spells"),
                      ...
                      StyleUtils.makeOptionalText(
                          ...
                          falseText:
                              "Haven't filled in all backstory information")
                    ]))
              ]),
            ));
  }
