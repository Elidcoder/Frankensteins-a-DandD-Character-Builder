...
import "package:frankenstein/features/character_creation/tabs/asi_feat_tab.dart"
    show AsiFeatTab;
import "package:frankenstein/features/character_creation/tabs/class_tab.dart"
    show ClassTab;
import "package:frankenstein/features/character_creation/tabs/equipment_tab.dart"
    show EquipmentTab;
import "package:frankenstein/features/character_creation/tabs/spells_tab.dart"
    show SpellsTab;
...
class EditCharacter extends State<EditACharacter> {
  ...
  void initState() {
    ...
    for (int i = 0; i < editableCharacter.classList.length; i++) {
      int classIndex = GlobalListManager()
          .classList
          .indexWhere((cls) => cls.name == editableCharacter.classList[i]);
      if (classIndex != -1 &&
          !classNames.contains(editableCharacter.classList[i])) {
        for (int level = 0;
            level < editableCharacter.levelsPerClass[count];
            level++) {
          if (GlobalListManager()
              .classList[classIndex]
              .gainAtEachLevel[level]
              .any((advancement) => advancement[0] == "ASI")) {
            expectedFeatOrASIs++;
          }
        }
        count++;
    ...
    int asiPointsUsed = editableCharacter.featsASIScoreIncreases
            .fold(0, (sum, increment) => sum + increment) ~/
        2; // Each ASI gives 2 points
    usedFeatOrASIs += asiPointsUsed;

    // Set remaining counts
    numberOfRemainingFeatOrASIs =
        (expectedFeatOrASIs - usedFeatOrASIs).clamp(0, expectedFeatOrASIs);
