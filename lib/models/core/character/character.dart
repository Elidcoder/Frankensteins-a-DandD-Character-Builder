...
class Character {
  ...
  Character getCopy() {
    return Character(
      languagesKnown: List<String>.from(languagesKnown),
      featuresAndTraits: List<String>.from(featuresAndTraits),
      classList: List<String>.from(classList),
      currency: Map<String, int>.from(currency),
      playerName: playerName,
      classLevels: List<int>.from(classLevels),
      inspired: inspired,
      mainToolProficiencies: List<String>.from(mainToolProficiencies),
      skillBonusMap: Map<String, int>.from(skillBonusMap),
      skillsSelected: Queue<String>.from(skillsSelected),
      languageChoices: Queue<String>.from(languageChoices),
      speedBonuses: Map<String, List<String>>.from(speedBonuses
          .map((key, value) => MapEntry(key, List<String>.from(value)))),
      ACList: ACList.map((list) => List<dynamic>.from(list)).toList(),
      ...
      savingThrowProficiencies: List<String>.from(savingThrowProficiencies),
      skillProficiencies: List<String>.from(skillProficiencies),
      maxHealth: maxHealth,
      characterExperience: characterExperience,
      classSkillsSelected: List<bool>.from(classSkillsSelected),
      race: race,
      subrace: subrace,
      optionalOnesStates: optionalOnesStates != null
          ? List<List<bool>>.from(
              optionalOnesStates!.map((list) => List<bool>.from(list)))
          : null,
      optionalTwosStates: optionalTwosStates != null
          ? List<List<bool>>.from(
              optionalTwosStates!.map((list) => List<bool>.from(list)))
          : null,
      raceAbilityScoreIncreases: List<int>.from(raceAbilityScoreIncreases),
      background: background,
      ...
      wisdom: wisdom,
      charisma: charisma,
      levelsPerClass: List<int>.from(levelsPerClass),
      allSelected: List<dynamic>.from(allSelected),
      classSubclassMapper: Map<String, String>.from(classSubclassMapper),
      featsASIScoreIncreases: List<int>.from(featsASIScoreIncreases),
      featsSelected: Map<Feat, int>.from(featsSelected),
      allSpellsSelected: List<Spell>.from(allSpellsSelected),
      allSpellsSelectedAsListsOfThings: List<List<dynamic>>.from(
          allSpellsSelectedAsListsOfThings
              .map((list) => List<dynamic>.from(list))),
      stackableEquipmentSelected:
          Map<String, int>.from(stackableEquipmentSelected),
      unstackableEquipmentSelected:
          List<dynamic>.from(unstackableEquipmentSelected),
      armourList: List<String>.from(armourList),
      weaponList: List<String>.from(weaponList),
      itemList: List<String>.from(itemList),
      equipmentSelectedFromChoices:
          List<dynamic>.from(equipmentSelectedFromChoices),
